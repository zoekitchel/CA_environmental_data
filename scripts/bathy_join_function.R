...
add_depth_columns <- function(lat_lon, grouping_var = "Site", ETOPO = F, USGS_socal=F, ETOPO_dist_200m = F){ 
  ...
  lat_lon_site_only <- na.omit(unique(lat_lon[,.(Site, Longitude, Latitude)]))
  
  if(ETOPO == T){
    
    #set square from which to extract bathy data from NOAA server
    marmap_bathy <- getNOAA.bathy(min(lat_lon_site_only$Longitude)-0.5, max(lat_lon_site_only$Longitude)+0.5, min(lat_lon_site_only$Latitude)-0.5, max(lat_lon_site_only$Latitude)+0.5, resolution = 0.000001) #bathymetry matrix
    
    #make new data.table of lat lon only to add new column to so no repeats if we have T for multiple bathy types
    lat_lon_site_only.c <- data.table(lat_lon_site_only)
    lat_lon_only.c <- lat_lon_site_only.c[,.(Longitude,Latitude)]
    
    #get depth info for specified lat lon coordinates
    lat_lon_site_only.c[,ETOPODepth := get.depth(marmap_bathy, lat_lon_only.c, locator=F)$depth]
    
    #merge with main 'return' output
    lat_lon_bathy <- lat_lon[lat_lon_site_only.c, on = c("Longitude","Latitude","Site"), all = T]
    
    #Rename
    lat_lon_bathy <- lat_lon_bathy %>% 
  ...
  #     lat_lon_site_only.sf <- st_as_sf(lat_lon_site_only, coords = c("Longitude", "Latitude"),
  #                                  crs = 4326) # WGS84 - a good default for unprojected coords)
  #   
  #     lat_lon_site_only.t <- st_transform(lat_lon_site_only.sf, crs = crs_Raster) #convert to raster projection for extraction
  #   
  #     #make new copy of lat lon only to add new column to so no repeats if we have T for multiple bathy types
  #     lat_lon_site_only.c <- copy(lat_lon_site_only)
  #     
  #   #match sites to depths
  #     lat_lon_site_only.c[, CDFW_200m:= extract(bathy_200m, lat_lon_site_only.t)]
  #     
  #     #link up (either to other bathy if it exists or just with lat lon coordinates)
  #       #merge with main 'return' output
  #     if(exists("lat_lon_bathy")){
  #     lat_lon_bathy <- lat_lon_bathy[lat_lon_site_only.c, on = c("Longitude","Latitude"), all = T]
  #     }else{
  #     lat_lon_bathy <- lat_lon[lat_lon_site_only.c, on = c("Longitude","Latitude"), all = T]
  #     }
  # }
  # 
  if(ETOPO_dist_200m == T){
    
    site_dist_200m <- data.table(dist2isobath(marmap_bathy, lat_lon_site_only$Longitude, lat_lon_site_only$Latitude, isobath = -200))
    
    #make new copy of lat lon only to add new column to so no repeats if we have T for multiple bathy types
    lat_lon_site_only.c <- copy(lat_lon_site_only)
    
    #match sites to distances
    lat_lon_site_only.c[, dist_200m_bath:= site_dist_200m$distance]
    
    #link up (either to other bathy if it exists or just with lat lon coordinates)
    #merge with main 'return' output
    if(exists("lat_lon_bathy")){
      lat_lon_bathy <- lat_lon_bathy[lat_lon_site_only.c, on = c("Longitude","Latitude"), all = T]
      lat_lon_bathy[,Site := NULL]
    }else{
      lat_lon_bathy <- lat_lon[lat_lon_site_only.c, on = c("Longitude","Latitude"), all = T]
      lat_lon_bathy[,Site := NULL]
    }
  ...
  if(USGS_socal == T){
    ...
    lat_lon_site_only.sf <- st_as_sf(lat_lon_site_only, coords = c("Longitude", "Latitude"), crs = 4326) %>% 
      st_transform(crs = crs(coverage_area))
    
    #add other lat lon values from previous projection
    lat_lon_site_only.sf$Latitude <- lat_lon_site_only$Latitude
    lat_lon_site_only.sf$Longitude <- lat_lon_site_only$Longitude
    
    #which lat long points link to which relevant DEM boundaries (DEM_ID)
    
    lat_lon_DEM_link <- st_intersection(lat_lon_site_only.sf, coverage_area)
    ...
    #Link to Site Names
    highres_socal$Site <- lat_lon_DEM_link$Site
    
    #Remove lat lon which were rounded in this process
    highres_socal <- highres_socal[,.(Site, highrez_depth)]
    
    #Take mean when values were pulled from two separate DEM files
    highres_socal_mean <- highres_socal[,.(highrez_depth = mean(highrez_depth)),.(Site)]
    
    print(paste0("Note that ",nrow(highres_socal_mean), " out of ",
                 nrow(lat_lon), " sites matched with the bathymetry dataset you selected."))
    
    
    #link up (either to other bathy if it exists or just with lat lon coordinates)
    #merge with main 'return' output
    if(exists("lat_lon_bathy")){
      lat_lon_bathy <- highres_socal_mean[lat_lon_bathy, on = c("Site"), all = T]
      lat_lon_bathy <- lat_lon_bathy %>% 
        rename(!!sym(grouping_var) := Site)
    }else{
      lat_lon_bathy <- left_join(lat_lon, highres_socal_mean, by = c("Site"))
      lat_lon_bathy <- lat_lon_bathy %>% 
        rename(!!sym(grouping_var) := Site)
    }
