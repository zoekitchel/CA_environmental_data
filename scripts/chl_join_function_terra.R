...
chl_join <- function(lat_lon, grouping_var = "Site", annual = F, min_max = F){
  ...
  if (!annual && !min_max) {
    lat_lon_variable_full_mean <- lat_lon_variable_full %>%
      group_by(Site, year) %>%
      mutate(chl_avg_annual = mean(chl_adj, na.rm = TRUE)) %>%
      ungroup()
    
    average_temperature_site <- lat_lon_variable_full_mean %>%
      group_by(Site) %>%
      summarise(mean_chl = mean(chl_avg_annual), .groups = "drop") %>%
