...
temp_join <- function(lat_lon, annual = F, min_max = F){
...
if(annual == F & min_max == F){
  lat_lon_site_variable_full <- lat_lon_site_variable_full %>%
    group_by(AR_complex, Site, year) %>%
    mutate(sst_avg_annual = mean(sst_adj, na.rm = T)) %>% ungroup()
  
  #Then take average of averages
  average_temperature_site <- group_by (AR_complex, Site) %>% summarise(mean_sst = mean(sst_avg_annual))
